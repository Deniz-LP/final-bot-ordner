...
module.exports = class ready extends Event {
  ...
  async run(client, message) {
    ...
    try {
      async function giverolelvl(rolelvlDB) {
        if (rolelvlDB) {
          if (rolelvlDB.role_id != null || rolelvlDB.role_id != undefined) {
            let role = message.guild.roles.cache.find((r) => r.id == "" + rolelvlDB.role_id);
            if ((message.guild.me.hasPermission("MANAGE_ROLES")) && (role.position < message.guild.me.roles.highest.position)) {
              ...
              if (randomChannel != undefined) {
                ...
                let lang = data.lang;
                if (lang == "de") {
                  randomChannel.send("Entschuldigen für Störung, aber Jamal bräuchte die Berechtigung MANAGE_ROLES, oder/und er bräuchte eine Rolle die höher ist als alle Levelrollen, <@" + message.guild.ownerID + ">.");
                } else if (lang == "en") {
                  randomChannel.send("Sorry for disturb, but Jamal would like to get the permission MANAGE_ROLES, or/and a role which is ranked highger than the role which should be higher than all leveling-roles, <@" + message.guild.ownerID + ">.");
                }
              }
            }
          }
        ...
        if (!(client.config.OWNERS.includes(message.author.id) || message.author.bot)) {
          ...
          client.users.fetch(client.config.OWNERS[0]).then((user) => {
            try {
              ...
              console.log("Error bei " + this.name + ": " + err)
            }
          });
          ...
          try {
            ...
            let lang = "en";
            if (args.length < 2) {
              return message.reply(client.utils.translation(lang, ["de", "Please specify a user and a message."]));
            }

            const targetUser = client.users.resolve(args[1]);
            if (!targetUser) {
              return message.reply(client.utils.translation(lang, ["de", "Invalid user specified."]));
            }

            let dmm = args.slice(2).join(" ");
            let ImageLink = "";
            try {
              message.attachments.forEach(attachment => {
                ImageLink += "\n" + attachment.proxyURL;
              ...
              console.log(`Error in command ${this.name}: ${err}`);
            }

            try {
              dmm += ImageLink;
              await targetUser.send(dmm);
              message.reply(client.utils.translation(lang, ["de", "Message sent! :)"]));
            } catch (err) {
              console.log(err)
              message.reply(client.utils.translation(lang, ["de", "This user has disabled DMs!"]));
            }
          } catch (e) {
      ...
      if (client.utils.isSleeping() == false && levelingsystem == true) {
        ...
        if (level && cd) {
          ...
          if (togglelvl.status === "on") {
            ...
            let XPneu = (parseInt(level.alleXP)) + GainedXP;
            let FullLVL = parseInt(level.fulllevel);
            let timen = BigInt(cd.cooldown);
            let normalexpneu = (parseInt(level.xp)) + GainedXP;
            let now = BigInt(Date.now());
            if (now >= timen) {
              let CooldownLevel = cd.cooldown;
              let COOLDOWN = Date + CooldownLevel;
              if (cooldownn == true) {
